...
const len = GIT_REPOSITORIES.length;
...
const printStat = stdout => {
    const statsRaw = stdout.match(regStat);
    let stats;
    let statModules;
    let statComponents;
...
const reg = /parsing/gm;
const regStat = /(statistics)[\s\S]*-----/gm;
const regStatModules = /(module {5}: )(\d+)/;
const regStatComponents = /(component {2}: )(\d+)/;
const loop = () => {
    if (i < len) {
